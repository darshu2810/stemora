...
import { getProjectProgress, studentName, type MockProject } from "@/lib/mock-data";
...
export function ProjectsGrid({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {projects.map((p) => {
        const { done, total, percent } = getProjectProgress(p.id);
        return (
          <Link
            key={p.id}
            href={`${basePath}/${p.id}`}
          ...
          >
            ...
            <p className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              {p.memberIds.length} students · Led by {studentName(p.leaderId)}
            </p>
            <div className="mt-3">
